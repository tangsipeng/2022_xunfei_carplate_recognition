...
from data.load_data_xunfei3 import CHARS, CHARS_DICT, LPRDataLoader
...
def get_parser():
    ...
    parser.add_argument('--lr_schedule', type=int, nargs='+', default=[4, 8, 12, 14, 16], help='schedule for learning rate.')
...
def train():
    ...
    #                     momentum=args.momentum, weight_decay=args.weight_decay)
    optimizer = optim.RMSprop([{'params':lprnet.parameters()}, {'params':STN.parameters()}], lr=args.learning_rate, alpha = 0.9, eps=1e-08,
                         momentum=args.momentum, weight_decay=args.weight_decay)    
    train_img_dirs = os.path.expanduser(args.train_img_dirs)
    test_img_dirs = os.path.expanduser(args.test_img_dirs)
    train_dataset = LPRDataLoader(train_img_dirs.split(','), args.img_size, args.lpr_max_len, True)
    test_dataset = LPRDataLoader(test_img_dirs.split(','), args.img_size, args.lpr_max_len, False)
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration !=0 and iteration % args.save_interval == 0:
            ...
            torch.save(STN.state_dict(), args.save_folder + 'STNet_' + '_iteration_' + repr(iteration) + '.pth')
